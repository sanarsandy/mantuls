  ...
  <div class="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <h1 class="text-3xl font-serif font-bold tracking-tight">PDF to Excel</h1>
          ...
          >
            ...
            <div class="space-y-4">
              <div class="w-16 h-16 bg-[var(--background)] border-2 border-[var(--foreground)] rounded-full flex items-center justify-center mx-auto mb-4">
                <Icon name="heroicons:table-cells" class="w-8 h-8 text-[var(--foreground)]" />
              </div>
              ...
              <div v-if="loading" class="space-y-2">
                 ...
                 <p class="text-sm font-mono">Converting to Excel...</p>
      ...
      <div class="lg:col-span-1 space-y-6">
        <div class="card-mono p-6 bg-[var(--background)]">
          ...
          <ul class="space-y-4 text-sm text-[var(--muted-foreground)]">
            ...
            <li class="flex items-start">
              <Icon name="heroicons:check" class="w-4 h-4 mr-2 text-[var(--foreground)] flex-shrink-0 mt-0.5" />
              Each table is saved as a separate sheet (if needed) or combined.
            </li>
            <li class="flex items-start">
              <Icon name="heroicons:check" class="w-4 h-4 mr-2 text-[var(--foreground)] flex-shrink-0 mt-0.5" />
...
const convertPDF = async () => {
  ...
  try {
    const response = await fetch(`${apiBase}/api/v1/tools/pdf-to-excel`, {
      method: 'POST',
      body: formData,
      headers: {
