  ...
  <div class="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <h1 class="text-3xl font-serif font-bold tracking-tight">Word to PDF</h1>
      ...
      <div class="lg:col-span-2 space-y-6">
        ...
        <div class="card-mono p-8">
          ...
          >
            ...
            <div class="space-y-4">
              ...
              <div v-if="!loading && !selectedFile">
                <h3 class="font-bold text-lg">Click to upload Word document</h3>
                <p class="text-[var(--muted-foreground)] text-sm font-mono">DOC or DOCX files (max 20MB)</p>
              </div>
              ...
              <div v-if="loading" class="space-y-2">
                 ...
                 <p class="text-sm font-mono">Converting to PDF...</p>
          ...
          <div v-if="selectedFile && !loading" class="mt-6 flex gap-4">
            ...
            <button @click="convertToPDF" class="btn-mono-primary flex-1 flex items-center justify-center">
        ...
        <div class="card-mono p-6 bg-[var(--background)]">
          ...
          <ul class="space-y-4 text-sm text-[var(--muted-foreground)]">
            <li class="flex items-start">
              ...
              Converts DOC and DOCX files to standard PDF format.
            </li>
            <li class="flex items-start">
              <Icon name="heroicons:check" class="w-4 h-4 mr-2 text-[var(--foreground)] flex-shrink-0 mt-0.5" />
            ...
            <li class="flex items-start">
              <Icon name="heroicons:check" class="w-4 h-4 mr-2 text-[var(--foreground)] flex-shrink-0 mt-0.5" />
              Perfect for sharing documents universally.
            </li>
          </ul>
        </div>
      </div>
...
const handleDrop = (e) => {
  ...
  if (droppedFile && isValid) {
    ...
    error.value = 'Please upload a Word document (DOC or DOCX).'
  }
}
...
const convertToPDF = async () => {
  ...
  try {
    const response = await fetch(`${apiBase}/api/v1/tools/word-to-pdf`, {
      method: 'POST',
      body: formData,
      headers: {
    ...
    error.value = err.message || 'Failed to convert Word to PDF. Please try again.'
  } finally {
    loading.value = false
  }
}
